...
class BasicLayer(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size):
        ...
        self.layer = net_utils.Conv2d_BatchNorm(in_planes, out_planes, kernel_size, same_padding=True)
...
class DenseBlock11(nn.Module):
    def __init__(self, nc):
        super(DenseBlock11, self).__init__()
        self.layer = self._make_layer(nc)

    def _make_layer(self, nc):
        layers = [BasicLayer(3, 32, 3), nn.MaxPool2d(kernel_size=2, stride=2),
                  BasicLayer(32, 64, 3), nn.MaxPool2d(kernel_size=2, stride=2)]
        if nc == 0:
            layers.append(BasicLayer(64, 128, 3))
            layers.append(BasicLayer(128, 64, 1))
            layers.append(BasicLayer(64, 128, 3))
        else:
            layers.append(MyLayer(64, 128, 3))
            layers.append(MyLayer(192, 64, 1))
            layers.append(BasicLayer(256, 128, 3))
...
class DenseBlock12(nn.Module):
    def __init__(self, nc):
        super(DenseBlock12, self).__init__()
        self.layer = self._make_layer(nc)

    def _make_layer(self, nc):
        layers = [nn.MaxPool2d(kernel_size=2, stride=2)]
        if nc == 0:
            layers.append(BasicLayer(128, 256, 3))
            layers.append(BasicLayer(256, 128, 1))
            layers.append(BasicLayer(128, 256, 3))
        else:
            layers.append(MyLayer(128, 256, 3))
            layers.append(MyLayer(384, 128, 1))
            layers.append(BasicLayer(512, 256, 3))
        return nn.Sequential(*layers)
...
class DenseBlock13(nn.Module):
    def __init__(self, nc):
        super(DenseBlock13, self).__init__()
        self.layer = self._make_layer(nc)

    def _make_layer(self, nc):
        layers = [nn.MaxPool2d(kernel_size=2, stride=2)]
        if nc == 0:
            layers.append(BasicLayer(256, 512, 3))
            layers.append(BasicLayer(512, 256, 1))
            layers.append(BasicLayer(256, 512, 3))
            layers.append(BasicLayer(512, 256, 1))
            layers.append(BasicLayer(256, 512, 3))
        else:
            layers.append(MyLayer(256, 512, 3))
            layers.append(MyLayer(768, 256, 1))
            layers.append(MyLayer(1024, 512, 3))
            layers.append(MyLayer(1536, 256, 1))
            layers.append(BasicLayer(1792, 512, 3))
        return nn.Sequential(*layers)
...
class DenseBlock4(nn.Module):
    ...
    def _make_layer(self):
        layers = [nn.MaxPool2d(kernel_size=2, stride=2)]
        layers.append(MyLayer(512, 1024, 3))
        layers.append(MyLayer(1536, 512, 1))
        layers.append(MyLayer(2048, 1024, 3))
        layers.append(MyLayer(3072, 512, 1))
        layers.append(BasicLayer(3584, 1024, 3))
        return nn.Sequential(*layers)
...
class Darknet19(nn.Module):
    def __init__(self, ncc):  # ncc is a list
        ...
        net_cfgs = [
            ...
            [(32, 3)],
            ['M', (64, 3)],  # item
            ['M', (128, 3), (64, 1), (128, 3)],  # item
            ['M', (256, 3), (128, 1), (256, 3)],  # item
            ['M', (512, 3), (256, 1), (512, 3), (256, 1), (512, 3)],  # item
            # conv2
        ...
        self.conv11 = DenseBlock11(ncc[0])
        self.conv12 = DenseBlock12(ncc[1])
        self.conv13 = DenseBlock13(ncc[2])
        c1 = 512
        ###########################################################################################################

        self.conv2 = DenseBlock4()
