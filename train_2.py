...
from darknet2 import Darknet19
...
parser.add_argument('--ncc', default=[1, 1, 1], type=list, help='liuyin14')                         # dense extent
parser.add_argument('--msdn', default='dense_model', type=str, help='liuyin18')                     # save model to model output dir (many h5 files)
parser.add_argument('--resume', default='dense_model/darknet19_voc07trainval_exp3_48.h5', type=str, help='liuyin22')    # load model
...
# pretrained_model = os.path.join('/home/saturn/yolo2-pytorch/models/training/', args.resume)
...
if use_tensorboard:
    summary_writer = SummaryWriter(os.path.join(cfg.TRAIN_DIR, 'runs', args.name))
else:
    summary_writer = None
...
for step in range(start_epoch * imdb.batch_per_epoch,
    ...
    # backward
    loss = net.loss
    # print(net.bbox_loss.data.cpu().numpy())
    ...
    if step > 0 and (step % imdb.batch_per_epoch == 0):
        ...
        temp = os.path.join('/home/saturn/yolo2-pytorch/models/training/', args.msdn)
        mkdir(temp)
        save_name = os.path.join(temp, '{}_{}.h5'.format(cfg.exp_name, imdb.epoch))
